...
<div class="stack stack-lg">
  ...
  <form action="${pageContext.request.contextPath}/admin/serveurs" method="get" class="search-form" id="search-form">
    <div class="search-input-wrapper">
      <input type="search" id="search-input" name="search" value="${searchQuery}" placeholder="Rechercher par adresse IP ou OS" class="form-control" />
    </div>
    ...
    <div class="table-responsive">
      <table class="data-table">
        <thead>
          <tr>
            <th>Adresse IP</th>
            <th>Système d'Exploitation (OS)</th>
            <th>Environnements Hébergés</th>
                ...
                <tr>
                  <td>
                    <div style="display: flex; align-items: center; gap: 8px;">
                      ...
                      <c:out value="${serveur.adressIP}" />
                    </div>
                  </td>
                  <td>
                    <span class="badge badge-outline">
                      <c:out value="${serveur.os}" />
                    </span>
                  </td>
                  <td>
                    <span class="badge badge-outline">
                      <c:out value="${serveur.nombreEnvironnements}" />
                    </span>
                  </td>
                  <td>
  ...
  <div class="modal-card">
    <header class="modal-header" style="align-items: flex-start;">
      <div class="modal-header-content">
        <div class="modal-header-icon">
          <i data-lucide="server" id="serveur-modal-icon"></i>
        </div>
        <div>
          <h2 id="serveur-modal-title">Ajouter un serveur</h2>
          <p id="serveur-modal-subtitle">Veuillez renseigner les informations du serveur.</p>
        </div>
      </div>
      <button type="button" class="modal-close" id="close-serveur-modal">
        <i data-lucide="x"></i>
      </button>
    </header>
    ...
    <form id="serveur-modal-form" action="${pageContext.request.contextPath}/admin/serveurs" method="post">
      ...
      <div class="modal-body stack stack-sm">
        ...
        <div class="form-group">
          ...
          <div class="input-with-icon">
            ...
            <input type="text" id="os" name="os" required class="form-control" placeholder="Ex: Ubuntu 22.04 LTS" />
          </div>
        </div>
      ...
      <div class="modal-actions" style="border-top: 1px solid var(--border-light); padding-top: 1.5rem;">
        ...
        <button type="submit" class="btn btn-primary" id="serveur-modal-submit">Enregistrer le serveur</button>
      </div>
    </form>
  </div>
</div>
...
<div id="delete-serveur-modal" class="modal-overlay" role="dialog" aria-modal="true">
  <div class="modal-card">
    <header class="modal-header">
      ...
      <button type="button" class="modal-close" id="delete-close-serveur-modal">
        <i data-lucide="x"></i>
      </button>
    </header>
    <form id="delete-serveur-form" action="${pageContext.request.contextPath}/admin/serveurs" method="post">
      ...
      <div class="modal-body">
        <p>Voulez-vous vraiment supprimer le serveur d'adresse IP <strong id="delete-serveur-ip"></strong> ?</p>
      </div>
      <div class="modal-actions">
        <button type="button" class="btn" style="background: transparent; border: 1px solid var(--border-light); color: var(--text-main);" id="delete-serveur-cancel">Annuler</button>
